...
from src.tranad_variants import is_tranad_variant_name, make_tranad_variant_class
torch.manual_seed(1)
...
def _require_dgl():
	...
	if dgl is None or GATConv is None:
		try:
			import dgl as _dgl
			from dgl.nn import GATConv as _GATConv
		except (ImportError, FileNotFoundError) as exc:
			raise ImportError('DGL is required for GDN and MTAD_GAT models.') from exc
		dgl = _dgl
		GATConv = _GATConv
	return dgl, GATConv

def _graph_to_data_device(graph, data):
	if graph.device != data.device:
...
class MTAD_GAT(nn.Module):
	...
	def forward(self, data, hidden):
		...
		self.g = _graph_to_data_device(self.g, data)
		data = data.view(self.n_window, self.n_feats)
		data_r = torch.cat((torch.zeros(1, self.n_feats, dtype=data.dtype, device=data.device), data))
		feat_r = self.feature_gat(self.g, data_r)
		data_t = torch.cat((torch.zeros(1, self.n_feats, dtype=data.dtype, device=data.device), data.t()))
		time_r = self.time_gat(self.g, data_t)
		data = torch.cat((torch.zeros(1, self.n_feats, dtype=data.dtype, device=data.device), data))
...
def __getattr__(name):
	if is_tranad_variant_name(name):
		model_class = make_tranad_variant_class(name)
